...
from data_spider_process.dataProscess import loadJson
from pprint import pprint as print

TOUR = ('200', '201', '202', '203')

def getTeamMember(team_name):
    members = loadJson("./data/Teams_member.json")
    ...
    ids = [int(player_id + tour) for player_id in members_id]
    return ids, names
...
def predict(teamA, teamB):
    ...
    output = f"""{teamA:^9s}: {[name[0] for name in teamA_names]}\n
            {teamB:^9s}: {[name[0] for name in teamB_names]}\n
            {'Predict':^9s}: {score}:{1-score} {result[0][score]:.2%}"""
